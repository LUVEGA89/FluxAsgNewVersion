...
namespace Reporting.Service.Core.CompletarFacturacionRecibida.Factory.Tipo
{
    public class Steuben2018 : ICreadorDB
    {
        ...
        public Steuben2018(string Empresa)
        {
        ...
        public Company CreateOcompany()
        {
            oCompany = new Company
            {
                ...
                Server = ConfigurationManager.AppSettings["DbServer.Steuben2018"],
                language = BoSuppLangs.ln_Spanish,
                UseTrusted = false,
                DbUserName = ConfigurationManager.AppSettings["DbUser.Steuben2018"],
                DbPassword = ConfigurationManager.AppSettings["DbPassword.Steuben2018"],
                LicenseServer = ConfigurationManager.AppSettings["LicenseServer.Steuben2018"],
                CompanyDB = ConfigurationManager.AppSettings["DbCompany.Steuben2018"],
                UserName = ConfigurationManager.AppSettings["SapUser.Steuben2018"],
                Password = ConfigurationManager.AppSettings["SapPassword.Steuben2018"]
            };
        ...
        public Documents FillPurchaseInvoice(Documents oPurchaseInvoice, FacturaCompra factura, ICreadorDB DbActual, string Referencia)
        {
            ...
            foreach (var concepto in factura.Conceptos)
            {
                ...
                if (concepto.Traslados != null)
                {
                    ...
                    {
                        oPurchaseInvoice.Lines.TaxCode = "IVA0C";
            ...
            if (factura.RetencionesOriginales.Count > 0)
            {
                if (factura.RetencionesOriginales[0] != null)
                {
                    ...
                    {
                        int BaseLineRetenciones = 0;
                        var Impuesto001 = factura.RetencionesOriginales.Where(h => h.Impuesto == "001" && h.TasaOCuota >= 0.099996m && h.TasaOCuota <= 0.100004m /*h.TasaOCuota == 0.100000m*/).ToArray();
                        var Impuesto002 = factura.RetencionesOriginales.Where(h => h.Impuesto == "002" && h.TasaOCuota >= 0.106600m && h.TasaOCuota <= 0.106700m /*h.TasaOCuota == 0.106667m*/).ToArray();
