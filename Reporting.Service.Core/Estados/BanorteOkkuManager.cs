...
{
    public class BanorteOkkuManager : DataRepository
    {
        public bool CoreInsertOkkuBanorte(string Cuenta, string FechaOperacion, string Fecha, string Referencia, string Descripcion, string Transaccion, string SucursalBanco, string Depositos, string Retiros, string Saldo, string Movimiento, string DescripcionDetallada, string Cheque)
        {
            DbCommand cmd = this.Database.GetStoredProcCommand("spInsertOkkuBanorte");
        ...
        public List<BanorteOkku> CoreGetEstadoOkkuBanorte()
        {
            List<BanorteOkku> Detalle = new List<BanorteOkku>();
            DbCommand cmd = this.Database.GetStoredProcCommand("spGetEstadoOkkuBanorte");
            cmd.CommandTimeout = 0;
            IDataReader dr = this.Database.ExecuteReader(cmd);
            while (dr.Read())
            {
                Detalle.Add(new BanorteOkku
                {
        ...
        public bool CoreUpdateDataOkkuBanorte(string Sequence, string Sucursal, string FecDiaVenta, string TipoPago, string Comentario)
        {
            DbCommand cmd = this.Database.GetStoredProcCommand("spUpdateAsociarDatosOkkuBanorte");
        ...
        public DataTable CoreGetReportExcelOkkuBanorte(string Del, string Al)
        {
            DbCommand cmd = this.Database.GetStoredProcCommand("spGetReportExcelOkkuBanorte");
